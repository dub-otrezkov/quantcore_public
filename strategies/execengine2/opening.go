...
func (e *Engine) openMarket(ctx context.Context, requests []model.OrderRequest, at time.Time) error {
	if len(requests) != 2 {
	...
	for i, placed := range results {
		...
		result = errors.Join(result, e.addOrder(placed.orderID, req))
	}
	if result != nil {
